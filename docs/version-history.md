...
| 2.1.204 | `Rer`     | -    | `cit`, `BY` | The thinking block's null return was wrapped in braces: `if(!cit&&!BY)return null;` became `if(!cit&&!BY){return null}`. The block was still located (it still begins `case"thinking":{if(!`), but the display patch's null-check regex expected the bare `)return null;` form and failed with "Could not parse null check variables". Updated the regex to accept both `)return null;` and `){return null}`, and switched the removal to strip the full matched construct (`nullCheckMatch[0]`) rather than reconstructing a fixed string. The other three patches (summary, force-display, tool-output) matched unchanged via dynamic regex. |
| 2.1.208 | `Rlt`, `gX` | -  | `Rlt`, `gX` | Only the tool-output patch broke. The grouped tool renderer's direct call `renderToolResultMessage?.(...)` was replaced by an indirection helper: `Hx(XAe,"renderToolResultMessage")?.(wAy,[],{verbose:!0,tools:ksr,theme:q0s})` — still the binary's only call with a literal `verbose:!0` (the other call site now uses a variable, `verbose:Yvd`). Updated the tool-output regex to accept either the bare `renderToolResultMessage?.(` call or `<helper>(<obj>,"renderToolResultMessage")?.(` before flipping the trailing `verbose:!0`→`verbose:!1`. The other three patches (display, summary, force-display) matched unchanged via dynamic regex. |
| 2.1.232 | `g8r`     | -    | `U2t`, `Mpe` | Only the tool-output patch broke again. The indirection helper call's 2nd argument changed from a paren-free literal (previously `[]`) to a call expression, `tk(C1e,"renderToolResultMessage")?.(auT,wne(YHi),{verbose:!0,...})` — the nested `wne(YHi)` contains parens that the regex's `[^)]*?` gap (meant to skip past the args before reaching `verbose:!0`) excluded by construction, so the match silently failed with "Pattern not found and no patched markers present." Replaced the gap with `(?:[^()]|\([^()]*\))*?`, which tolerates exactly one level of nested parens while still stopping at the first `verbose:!0`/`verbose:!1` it finds. The other call site (`verbose:vUm`, a variable) still correctly doesn't match either regex. The other three patches (display, summary, force-display) matched unchanged via dynamic regex. |
